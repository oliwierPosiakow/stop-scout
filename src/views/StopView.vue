...
const stopName = route.query.name?.toString();
...
<template>
  <div class="flex flex-col gap-4 px-4 py-4">
    ...
    <section>
      <h2 class="text-ios-2xl font-bold text-black dark:text-white mb-1">
        {{ stopName }}
      </h2>
    ...
    <div v-if="isEditing" class="ios-card border-2 border-blue-500 dark:border-blue-400">
      ...
      <div class="space-y-3">
        <div>
          ...
          <input v-model="stopAlias" placeholder="np. Pod pracą" class="ios-input w-full" />
        </div>
        <div>
        ...
        <div class="flex gap-2 pt-2">
          ...
          <button @click="isEditing = false" class="flex-1 ios-button-secondary text-ios-base">
            Anuluj
          </button>
        </div>
      </div>
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-12">
      <div
        class="animate-spin rounded-full h-12 w-12 border-4 border-gray-300 dark:border-slate-700 border-t-blue-500"
      ></div>
    ...
    <div v-else-if="error" class="ios-card border-l-4 border-red-500 bg-red-50 dark:bg-red-900/20">
      ...
      <button @click="fetchDepartures(stopId)" class="ios-button-primary w-full text-ios-base">
        Spróbuj ponownie
      </button>
    </div>
